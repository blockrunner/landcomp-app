...
class ChatSession extends Equatable {
  ...
  factory ChatSession.fromJson(Map<String, dynamic> json) {
    ...
    print('📂 ChatSession.fromJson: Processing ${messagesJson.length} messages');
    ...
    for (final messageJson in messagesJson) {
      ...
      if (imageBytes != null) {
        print('📂 ChatSession.fromJson: Message ${messageData['id']} has ${(imageBytes as List).length} images in JSON');
      }
    }
    ...
    for (final message in messages) {
      if (message.attachments != null && message.attachments!.isNotEmpty) {
        ...
        print('📂 ChatSession.fromJson: Message ${message.id} has ${imageAttachments.length} images after deserialization');
      }
    }
  ...
  int get aiMessageCount => messages.where((m) => m.type == MessageType.ai).length;
