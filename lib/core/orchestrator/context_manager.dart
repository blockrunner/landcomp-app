...
class ContextManager {
  ...
  Future<RequestContext> buildContext({
    ...
    print('🔍 Building context for message: ${userMessage.substring(0, userMessage.length > 50 ? 50 : userMessage.length)}...');
    ...
    print('✅ Context built: ${conversationHistory.length} messages, ${finalAttachments?.length ?? 0} attachments, language: $language');
    return context;
  }
  ...
  bool _checkRecentImages(List<Message> history) {
    ...
    for (final message in recentMessages) {
      if (message.attachments != null && message.attachments!.isNotEmpty) {
        final hasImages = message.attachments!.any((attachment) => attachment.isImage);
        if (hasImages) {
  ...
  Map<String, dynamic> _buildMetadata(List<Message> history, String? currentAgentId) {
    final metadata = <String, dynamic>{
      'conversation_length': history.length,
      'user_message_count': history.where((m) => m.type == MessageType.user).length,
      'ai_message_count': history.where((m) => m.type == MessageType.ai).length,
      'system_message_count': history.where((m) => m.type == MessageType.system).length,
      'current_agent_id': currentAgentId,
      'has_attachments': history.any((m) => m.attachments != null && m.attachments!.isNotEmpty),
      'has_image_analyses': history.any((m) => m.imageAnalysis != null && m.imageAnalysis!.isNotEmpty),
    };
  ...
  List<Attachment>? _extractAttachmentsFromHistory(List<Message> history) {
    ...
    for (int i = recentMessages.length - 1; i >= 0; i--) {
      ...
      if (message.attachments != null && message.attachments!.isNotEmpty) {
        print('📸 Found attachments in message ${message.id}: ${message.attachments!.length} items');
