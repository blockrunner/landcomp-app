...
class AIService {
  ...
  Future<String> sendToOpenAI({
    ...
    String model =
        'gpt-4o', // Changed from 'gpt-4' to 'gpt-4o' for Vision API support
    ...
    try {
      ...
      final response = await _dio.post<Map<String, dynamic>>(
        ...
        data: {
          'model': model,
          'messages': await _buildOpenAIMessages(
            systemPrompt,
            message,
            conversationHistory,
            maxHistoryMessages,
  ...
  Future<List<Map<String, dynamic>>> _buildOpenAIMessages(
    ...
    final enhancedSystemPrompt = _enhanceSystemPromptWithLanguage(
      systemPrompt,
      message,
      conversationHistory,
    );
  ...
  String _enhanceSystemPromptWithLanguage(
    ...
    final detectedLanguage = _detectUserLanguage(
      currentMessage,
      conversationHistory,
    );
...
4. If the user sent an image earlier, remember it and take it into account in your answers.''';
  ...
  Future<Uint8List> _compressImage(
    ...
    try {
      ...
      final targetPixels =
          (maxSizeKb * 1024 * 10) / 3; // RGB = 3 bytes per pixel
  ...
  Future<String> sendToGemini({
    ...
    try {
      ...
      if (e.response?.statusCode == 429) {
        ...
        throw Exception(
          'Google Gemini API access forbidden. Check your API key permissions.',
        );
      } else if (e.type == DioExceptionType.connectionTimeout) {
        throw Exception(
          'Connection timeout. Please check your internet connection.',
      ...
      if (response.statusCode == 200) {
        ...
        for (var i = 0; i < responseParts.length; i++) {
          ...
          if (part['text'] != null) {
            ...
            final mimeType =
                (inlineData['mime_type'] ?? inlineData['mimeType']) as String;
...
class SmartAIResponse {
  ...
  factory SmartAIResponse.outOfScope({required String message}) {
    return SmartAIResponse._(
      isSuccess: false,
      isOutOfScope: true,
      isError: false,
  ...
  factory SmartAIResponse.error({required String message}) {
    return SmartAIResponse._(
      isSuccess: false,
      isOutOfScope: false,
      isError: true,
