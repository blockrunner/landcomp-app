...
class ImageUtils {
  ...
  static bool validateImage(Uint8List imageBytes) {
    try {
      ...
      if (imageBytes.length > maxImageSize) {
        print(
          '❌ Image too large: ${imageBytes.length} bytes (max: $maxImageSize)',
        );
        return false;
      }
  ...
  static Future<List<Uint8List>> compressImages(List<Uint8List> images) async {
    ...
    print(
      '🗜️ Compression complete: ${images.length} -> ${compressedImages.length} images',
    );
  ...
  static List<Uint8List> validateImages(List<Uint8List> images) {
    ...
    print(
      '🔍 Validation complete: ${images.length} -> ${validImages.length} valid images',
    );
    return validImages;
  }
}
